...
type EditVeiculoFormData = z.infer<typeof editVeiculoSchema>
...
export function EditVeiculoForm({ veiculo }: EditVeiculoFormProps) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)

  const form = useForm<EditVeiculoFormData>({
    ...
    defaultValues: {
      ...
      potencia: veiculo.potencia ? veiculo.potencia.toString() : "",
      valorPorKm: veiculo.valorPorKm.toString() || "",
      ativo: veiculo.ativo
    }
  })

  const onSubmit = async (data: EditVeiculoFormData) => {
    try {
      setLoading(true)
      ...
      if (!response.ok) {
        ...
        alert(error.error || 'Erro ao atualizar veículo')
        return
      }
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                  <div className="space-y-0.5">
                    ...
                    <FormDescription>
                      Controla se o veículo pode ser usado em novas despesas
                    </FormDescription>
                  </div>
                  <FormControl>
                    <Switch
